...
import {
  ...
  faq,
  relatedGuides,
  type SmallSolidWoodDesk,
} from "@/data/guides/best-small-solid-wood-desk";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-small-solid-wood-desk",
  image: fallbackHeroImage,
  type: "article",
});

function ProductSection({ product }: { product: SmallSolidWoodDesk }) {
  return (
        ...
        <div className="flex flex-col sm:flex-row gap-5 p-5">
          ...
          <div className="flex-1 min-w-0 flex flex-col gap-3">
            <div className="flex flex-wrap gap-2">
              ...
              <span className="text-xs px-2.5 py-1 rounded-lg bg-gray-100 text-ink-secondary font-medium">{product.width} wide</span>
              <span className="text-xs px-2.5 py-1 rounded-lg bg-gray-100 text-ink-secondary font-medium">{product.depth} deep</span>
              <span className="text-xs px-2.5 py-1 rounded-lg bg-gray-100 text-ink-secondary font-medium">{product.material}</span>
              {product.drawers && (
                <span className="text-xs px-2.5 py-1 rounded-lg bg-gray-100 text-ink-secondary font-medium">{product.drawers}</span>
              )}
            </div>
            <div>
...
export default async function BestSmallSolidWoodDeskPage() {
  const guide = await getPublicGuideBySlug("best-small-solid-wood-desk");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-small-solid-wood-desk` },
    about: [
      { "@type": "Thing", name: "Small Solid Wood Desk" },
      { "@type": "Thing", name: "Bedroom Desk" },
      { "@type": "Thing", name: "Apartment Desk" },
    ],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Small Solid Wood Desks for Bedroom", item: `${SITE_URL}/guide/best-small-solid-wood-desk` },
    ],
  };
  ...
  const itemListSchema = {
    "@context": "https://schema.org",
    "@type": "ItemList",
    name: "Best Small Solid Wood Desks for Bedroom 2026",
    description: "9 best small solid wood desks for bedroom and apartment, under 48 inches wide.",
    numberOfItems: products.length,
    itemListElement: products.map((p) => ({
      "@type": "ListItem",
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-small-solid-wood-desk#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Small Solid Wood Desks for Bedroom</span>
        </nav>
        ...
        <header className="mb-8">
          ...
          <p className="text-lg text-ink-secondary leading-relaxed max-w-3xl">
            Small solid wood desks (under 48 inches wide) are harder to find than large ones because most solid wood furniture targets full-size home offices. This guide covers 9 genuine solid wood options under 48 inches wide, with depth versus width ergonomics guidance and monitor arm compatibility notes for compact setups.
          </p>
          <div className="flex flex-wrap items-center gap-4 mt-5 pt-5 border-t border-border">
            ...
            <div className="flex items-center gap-1.5 text-xs text-ink-muted">
              ...
              9 solid wood desks under 48 inches evaluated
            </div>
          </div>
        </header>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          <Image src={heroImg} alt="Best small solid wood desks for bedroom" width={900} height={420} className="w-full object-cover max-h-[420px]" priority unoptimized />
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-brand text-white">
                  ...
                  <th className="text-left px-4 py-3 font-semibold hidden sm:table-cell">Width</th>
              ...
              <tbody>
                {atAGlanceRows.map((row, i) => (
                  <tr key={row.id} className={i % 2 === 0 ? "bg-white" : "bg-bg"}>
                    ...
                    <td className="px-4 py-3 text-ink-secondary text-xs hidden sm:table-cell">{row.width}</td>
                    <td className="px-4 py-3 text-ink-secondary text-xs hidden sm:table-cell">{row.material}</td>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>
            Small solid wood desks serve three primary use cases: bedroom offices where floor space is shared with a bed and wardrobe, apartment setups where the desk must double as a dining or general table, and minimalist home offices where the goal is a clean visible surface without visual bulk. The products on this list are all under 48 inches wide and verified to use genuine hardwood construction. For comparison with larger solid wood desks, the{" "}
            <Link href="/guide/best-solid-wood-desk" className="text-brand hover:underline">solid wood desk guide</Link>{" "}
            covers options from 42 to 60+ inches. For solid wood with drawer storage, the{" "}
            <Link href="/guide/best-wooden-desk-with-drawers" className="text-brand hover:underline">wooden desk with drawers guide</Link>{" "}
            covers compact and full-size options.
          </p>
        </section>
        ...
        <h2 className="text-2xl font-bold text-ink mb-8 tracking-tight">{products.length} Best Small Solid Wood Desks for Bedroom</h2>
