...
import {
  ...
  faq,
  relatedGuides,
  type WoodenDeskWithDrawers,
} from "@/data/guides/best-wooden-desk-with-drawers";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-wooden-desk-with-drawers",
  image: fallbackHeroImage,
  type: "article",
});

function ProductSection({ product }: { product: WoodenDeskWithDrawers }) {
  return (
...
export default async function BestWoodenDeskWithDrawersPage() {
  const guide = await getPublicGuideBySlug("best-wooden-desk-with-drawers");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-wooden-desk-with-drawers` },
    about: [
      { "@type": "Thing", name: "Wooden Desk with Drawers" },
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Wooden Desks with Drawers", item: `${SITE_URL}/guide/best-wooden-desk-with-drawers` },
    ],
  };
  ...
  const itemListSchema = {
    "@context": "https://schema.org",
    "@type": "ItemList",
    name: "Best Wooden Desks with Drawers 2026",
    description: "7 best wooden desks with drawers in solid wood construction.",
    numberOfItems: products.length,
    itemListElement: products.map((p) => ({
      "@type": "ListItem",
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-wooden-desk-with-drawers#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Wooden Desks with Drawers</span>
        </nav>
        ...
        <header className="mb-8">
          ...
          <p className="text-lg text-ink-secondary leading-relaxed max-w-3xl">
            The most common quality trap in the wooden desk with drawers category is a solid wood frame with MDF drawer boxes. This guide covers 7 desks where both the desk body and drawer construction use honest solid wood or clearly documented engineered wood, with guidance on drawer joint types and slide quality to look for.
          </p>
          <div className="flex flex-wrap items-center gap-4 mt-5 pt-5 border-t border-border">
            ...
            <div className="flex items-center gap-1.5 text-xs text-ink-muted">
              ...
              7 wooden desks with drawers evaluated
            </div>
          </div>
        </header>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          <Image src={heroImg} alt="Best wooden desks with drawers" width={900} height={420} className="w-full object-cover max-h-[420px]" priority unoptimized />
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-brand text-white">
                  ...
                  <th className="text-left px-4 py-3 font-semibold hidden sm:table-cell">Drawers</th>
              ...
              <tbody>
                {atAGlanceRows.map((row, i) => (
                  <tr key={row.id} className={i % 2 === 0 ? "bg-white" : "bg-bg"}>
                    ...
                    <td className="px-4 py-3 text-ink-secondary text-xs hidden sm:table-cell">{row.drawers}</td>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>
            IC International Concepts is the most consistently documented solid wood brand in the Amazon desk market. Their entire product line uses parawood (rubberwood) throughout, including drawer boxes. For buyers who need verified solid hardwood construction across the whole desk, IC is the starting point. For desks with more drawers in a compact footprint, the best wooden gaming desks also include drawer-focused options in our{" "}
            <Link href="/guide/best-wooden-gaming-desk" className="text-brand hover:underline">wooden gaming desk guide</Link>
            . For executive-format desks with file drawers,{" "}
            <Link href="/guide/best-wood-executive-desk" className="text-brand hover:underline">the wood executive desk guide</Link>{" "}
            covers the Sauder Palladia and Bush Business options.
          </p>
        </section>
        ...
        <h2 className="text-2xl font-bold text-ink mb-8 tracking-tight">{products.length} Best Wooden Desks with Drawers</h2>
