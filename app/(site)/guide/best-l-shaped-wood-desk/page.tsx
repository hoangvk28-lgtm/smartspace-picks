...
import {
  ...
  relatedGuides,
  type LShapedDesk,
} from "@/data/guides/best-l-shaped-wood-desk";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-l-shaped-wood-desk",
  image: fallbackHeroImage,
  type: "article",
});

function ProductSection({ product }: { product: LShapedDesk }) {
  return (
...
export default async function BestLShapedWoodDeskPage() {
  const guide = await getPublicGuideBySlug("best-l-shaped-wood-desk");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-l-shaped-wood-desk` },
    about: [
      { "@type": "Thing", name: "L-Shaped Wood Desk" },
      { "@type": "Thing", name: "Home Office Desk" },
      { "@type": "Thing", name: "Corner Desk" },
    ],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best L-Shaped Wood Desks for Home Office", item: `${SITE_URL}/guide/best-l-shaped-wood-desk` },
    ],
  };
  ...
  const itemListSchema = {
    "@context": "https://schema.org",
    "@type": "ItemList",
    name: "Best L-Shaped Wood Desks for Home Office 2026",
    description: "7 best L-shaped wood desks for home office including standing options.",
    numberOfItems: products.length,
    itemListElement: products.map((p) => ({
      "@type": "ListItem",
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-l-shaped-wood-desk#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best L-Shaped Wood Desks for Home Office</span>
        </nav>
        ...
        <header className="mb-8">
          ...
          <p className="text-lg text-ink-secondary leading-relaxed max-w-3xl">
            The L-shaped desk format suits corner spaces and dual-monitor setups where a straight desk creates dead corner space instead of using it. This guide covers 7 L-shaped wood desks with honest notes on the reversible return configuration trap, corner dead zone math, and left versus right return selection.
          </p>
          <div className="flex flex-wrap items-center gap-4 mt-5 pt-5 border-t border-border">
            ...
            <div className="flex items-center gap-1.5 text-xs text-ink-muted">
              ...
              7 L-shaped wood desks evaluated
            </div>
          </div>
        </header>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          <Image src={heroImg} alt="Best L-shaped wood desks for home office" width={900} height={420} className="w-full object-cover max-h-[420px]" priority unoptimized />
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-brand text-white">
                  ...
                  <th className="text-left px-4 py-3 font-semibold hidden sm:table-cell">Standing</th>
              ...
              <tbody>
                {atAGlanceRows.map((row, i) => (
                  <tr key={row.id} className={i % 2 === 0 ? "bg-white" : "bg-bg"}>
                    ...
                    <td className="px-4 py-3 text-ink-secondary text-xs hidden sm:table-cell">{row.size}</td>
                    <td className="px-4 py-3 text-ink-secondary text-xs hidden sm:table-cell">{row.standing}</td>
                    <td className="px-4 py-3">
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>
            Every L-shaped desk on Amazon uses the term &ldquo;reversible&rdquo; in the title. This means the return leg (shorter side) can be set to either the left or right at assembly. It does not mean you can switch sides after the desk is fully assembled. Determine your room layout before assembly to avoid disassembly. Most L-shaped desks use engineered wood construction; for solid hardwood options, the{" "}
            <Link href="/guide/best-solid-wood-desk" className="text-brand hover:underline">solid wood desk guide</Link>{" "}
            covers the verified hardwood options. For standing desk configurations, the{" "}
            <Link href="/guide/best-solid-wood-standing-desk" className="text-brand hover:underline">solid wood standing desk guide</Link>{" "}
            covers motorized L-shaped options with real wood tops.
          </p>
        </section>
        ...
        <h2 className="text-2xl font-bold text-ink mb-8 tracking-tight">{products.length} Best L-Shaped Wood Desks for Home Office</h2>
